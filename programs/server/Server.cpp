...
int Server::run()
{
    ...
    {
        std::cout << DBMS_NAME << " server version " << VERSION_STRING << " (" << BILI_VERSION_STRING << ")"
                  << "." << std::endl;
        return 0;
    }
...
if (ThreadFuzzer::instance().isEffective())
    ...
    size_t uki_meta_cache_elements_size = config().getUInt64("unique_key_index_meta_cache_elements_size", 5000); /// 5000
    size_t uki_data_cache_size = config().getUInt64("unique_key_index_data_cache_size", 1073741824); /// 1GB
    global_context->setUniqueKeyIndexCache(uki_meta_cache_size, uki_meta_cache_elements_size);
