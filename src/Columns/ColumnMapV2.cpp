...
bool ColumnMapV2::tryInsert(const Field & x)
{
...
char * ColumnMapV2::serializeValueIntoMemory(size_t n, char * memory) const
{
...
WeakHash32 ColumnMapV2::getWeakHash32() const
{
...
void ColumnMapV2::insertFrom(const IColumn & src, size_t n)
#else
void ColumnMapV2::doInsertFrom(const IColumn & src, size_t n)
#endif
{
    nested->insertFrom(assert_cast<const ColumnMapV2 &>(src).getNestedColumn(), n);
}

#if !defined(DEBUG_OR_SANITIZER_BUILD)
void ColumnMapV2::insertManyFrom(const IColumn & src, size_t position, size_t length)
#else
void ColumnMapV2::doInsertManyFrom(const IColumn & src, size_t position, size_t length)
#endif
{
    assert_cast<ColumnArray &>(*nested).insertManyFrom(assert_cast<const ColumnMapV2 &>(src).getNestedColumn(), position, length);
}

#if !defined(DEBUG_OR_SANITIZER_BUILD)
void ColumnMapV2::insertRangeFrom(const IColumn & src, size_t start, size_t length)
#else
void ColumnMapV2::doInsertRangeFrom(const IColumn & src, size_t start, size_t length)
#endif
{
...
ColumnPtr ColumnMapV2::replicate(const Offsets & offsets) const
{
    ...
    return ColumnMapV2::create(std::move(replicated));
}
...
#else
int ColumnMapV2::doCompareAt(size_t n, size_t m, const IColumn & rhs, int nan_direction_hint) const
#endif
{
...
size_t ColumnMapV2::capacity() const
{
...
void ColumnMapV2::getExtremes(Field & min, Field & max) const
{
    ...
    MapV2 map_min_value(nested_min_value.begin(), nested_min_value.end());
    MapV2 map_max_value(nested_max_value.begin(), nested_max_value.end());
...
namespace
{
DataTypePtr getBaseDataType(const TypeIndex & type_index)
{
    switch (type_index)
    {
    ...
    for (size_t row_num = 0; row_num < size(); ++row_num)
    {
        ...
        for (size_t element_num = 0; element_num < elements_size; ++element_num)
        {
            ...
            if (it == names_and_columns.end())
            {
                ...
                new_column->insert(v);
                names_and_columns.insert({key, new_column->assumeMutable()});
            }
            else
            {
                ...
                /// we should fill it with default values till the size of this column be the same as the max column's.
                /// FIXME: Here is a bug, if a map have same key, clickhouse client will be blocked ?
