...
/** Column that store map data in implicit columns.
 */
class ColumnMapV2 final : public COWHelper<IColumn, ColumnMapV2>
{
private:
    friend class COWHelper<IColumn, ColumnMapV2>;

    WrappedPtr nested;

    mutable ColumnsWithTypeAndName columns;
    explicit ColumnMapV2(MutableColumnPtr && nested_);

    ColumnMapV2(const ColumnMapV2 &) = default;

public:
    /** Create immutable column using immutable arguments. This arguments may be shared with other columns.
      * Use IColumn::mutate in order to make mutable column and mutate shared nested columns.
      */
    using Base = COWHelper<IColumn, ColumnMapV2>;
    ...
    static Ptr create(const ColumnPtr & keys, const ColumnPtr & values, const ColumnPtr & offsets)
    {
        ...
        return ColumnMapV2::create(nested_column);
    }

    static Ptr create(const ColumnPtr & column) { return ColumnMapV2::create(column->assumeMutable()); }
    ...
    TypeIndex getDataType() const override { return TypeIndex::MapV2; }
    ...
    void addColumn(const ColumnWithTypeAndName & column) { columns.emplace_back(column); }
    void setColumns(const ColumnsWithTypeAndName & new_columns);
    const ColumnsWithTypeAndName & getColumns() const { return columns; }

    ColumnPtr compress() const override;

    const IColumn & getColumn(int index) const { return *columns[index].column; }

    void constructImplicitColumns() const;

    static DataTypePtr getBaseDataType(const TypeIndex & type_index);
};
