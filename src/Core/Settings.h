...
#define COMMON_SETTINGS(M) \
    ...
    M(RemoteQueryTimeOutMode, remote_query_timeout_mode, RemoteQueryTimeOutMode::IMMEDIATE_THROW, "What to do if the remote query timeout exceeded.", 0) \
    ...
    M(UInt64, replication_alter_mutation_sync, 1, "Wait for mutations caused by ALTER Commands (exclude PARTITION and MUTATION Commands, like DROP PARTITION, DROP PART, MOVE PARTITION, DELETE, UPDATE) to finish. 0 - do not wait, 1 - wait for the mutation only of itself, 2 - wait for everyone.", 0) \
    \
    ...
    M(Seconds, distributed_replica_remote_error_half_life, DBMS_CONNECTION_POOL_WITH_REMOTE_EXCEPTION_DEFAULT_DECREASE_ERROR_PERIOD, "Time period reduces replica remote error counter by 2 times.", 0) \
