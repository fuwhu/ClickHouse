...
BidirectionalStorageRocksDB::BidirectionalStorageRocksDB(
    ...
    : log(getLogger("BidirectionalStorageRocksDB(" + dict_ + ")"))
    , dict(dict_)
    , uuid(uuid_)
...
void BidirectionalStorageRocksDB::closeDB(bool clean)
{
    if (rocksdb)
    {
        ...
        {
            String rocksdb_dir = std::filesystem::path(local_base_path) / (dict + "." + uuid);
            LOG_INFO(log, "Clean up local rocksdb directory {}", rocksdb_dir);
...
BidirectionalStorageRemote::BidirectionalStorageRemote(
    const String & dict_, const String & target_, const Poco::Util::AbstractConfiguration & global_conf)
    : log(getLogger("BidirectionalStorageRemote(" + dict_ + ")")), dict(dict_), target(target_)
{
    initGRPCLogging(global_conf);
    initClient();
    checkDict();
}
...
std::vector<UInt64> BidirectionalStorageRemote::getValues(const std::vector<StringRef> & keys, size_t start, size_t size)
{
    ...
    while (cursor < size)
    {
        ...
        req.mutable_keys()->Reserve(static_cast<int>(needs));
...
std::vector<String> BidirectionalStorageRemote::getKeys(const std::vector<UInt64> & values, size_t start, size_t size)
{
    ...
    while (cursor < size)
    {
        ...
        req.set_dict(dict);
        size_t needs = std::min(MAX_PROXY_BATCH_SIZE, size - cursor);
        req.mutable_values()->Reserve(static_cast<int>(needs));
        for (auto row : collections::range(needs))
        {
