...
BidirectionalDictionarySource::BidirectionalDictionarySource(
    ...
    : log(getLogger("BidirectionalDictionarySource")), dict_struct(dict_struct_), configuration(configuration_), sample_block(sample_block_)
{
...
{
#if USE_GRPC
    auto remote_storage = std::make_shared<BidirectionalStorageRemote>(
        configuration.dict_name, configuration.dict_proxy_address, configuration.global_context->getConfigRef());
#    if USE_ROCKSDB
    storage = std::make_shared<BidirectionalStorageRocksDB>(
        configuration.dict_name, configuration.uuid, configuration.dict_rocksdb_path, remote_storage);
#    else
    storage = remote_storage;
#    endif
#else
    throw Exception(ErrorCodes::NOT_IMPLEMENTED, "No available source for BidirectionalDictionary");
#endif
}

QueryPipeline BidirectionalDictionarySource::loadKeys(const Columns & key_columns, const std::vector<size_t> & requested_rows)
{
    ...
    if (value_column->isNullAt(0))
    {
        ...
        return QueryPipeline(std::make_shared<BidirectionalSource>(storage, sample_block, std::move(keys), std::vector<UInt64>()));
    }
    /// values -> keys
    else if (key_column->isNullAt(0))
    {
        ...
        return QueryPipeline(std::make_shared<BidirectionalSource>(storage, sample_block, std::vector<StringRef>(), std::move(values)));
    }
...
void registerDictionarySourceBidirectional(DictionarySourceFactory & factory)
{
                                  ...
                                  bool /* created_from_ddl */) -> DictionarySourcePtr
    {
        ...
        BidirectionalDictionarySource::Configuration configuration{
            .uuid = uuid,
            .dict_name = dict,
            .dict_proxy_address = proxy_addr,
            .dict_rocksdb_path = rocksdb_path,
            .global_context = global_context};

        return std::make_shared<BidirectionalDictionarySource>(dict_struct, configuration, sample_block);
    };
