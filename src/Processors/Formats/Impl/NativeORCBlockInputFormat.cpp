...
ORCInputStream::ORCInputStream(SeekableReadBuffer & in_, size_t file_size_, bool use_prefetch_) : in(in_), file_size(file_size_), supports_read_at(use_prefetch_ && in.supportsReadAt()), pool(ORCMemoryPool::instance()) {
...
void ORCInputStream::read(void * buf, uint64_t length, uint64_t offset)
{
    if (supports_read_at)
    {
        ...
        {
            uint64_t bytes_to_read = length - bytes_read;
            uint64_t n = in.readBigAt(reinterpret_cast<char *>(buf) + bytes_read, bytes_to_read, offset + bytes_read, nullptr);
            bytes_read += n;
        }
    }
    else
    ...
    if (!inCurrentRange(offset)) {
        ...
        if (buffer->empty()) {
            ...
            if (slice_lenth > new_buffer_size)
                throw Exception(ErrorCodes::LOGICAL_ERROR, "the buffer end should not execeed the end of current range.");
...
{
    auto * seekable_in = dynamic_cast<ReadBufferFromFileBase *>(&in);

    if (seekable_in && settings.seekable_read && seekable_in->getFileSize())
        return std::make_unique<ORCInputStream>(*seekable_in, seekable_in->getFileSize(), use_prefetch);
    else
...
NativeORCBlockInputFormat::NativeORCBlockInputFormat(ReadBuffer & in_, Block header_, const FormatSettings & format_settings_, bool use_prefetch_, size_t min_bytes_for_seek_)
    ...
    , log(getLogger("NativeORCBlockInputFormat"))
{
...
void NativeORCBlockInputFormat::prepareFileReaderAndMetadata()
{
    ...
    if (use_prefetch)
        getFileReaderAndSchema(*shared_in_buf, file_reader, file_schema, format_settings, use_prefetch, min_bytes_for_seek, is_stopped);
    else
...
void NativeORCBlockInputFormat::checkStripeFiltered(std::vector<uint32_t> & stripes)
{
    ...
    auto remove_constant_column = [&](std::list<UInt64> &include_indices_, Block &sample_block, uint64_t stripe_index)
    {
        Stopwatch stop_watch;
        ...
        for (auto it = include_indices_.begin(); it != include_indices_.end(); )
        {
            ...
            auto is_constant = checkStripeColumnConstantness(stripe_index, col_index, constant_value);
            if (is_constant)
            {
    ...
    for (auto stripe : stripes)
    {
        ...
        if (prewhere_info)
        {
            ...
            remove_constant_column(prewhere_include_indices_for_stripe, prewhere_header_for_stripe, stripe);
            ...
            while (next_prewhere_stripe_reader->hasPendingData())
            {
                ...
                if (executePrewhere(pre_res_columns, filter_holder, filter, true) != 0)
...
void NativeORCBlockInputFormat::prefetchStripes()
{
    ...
    if (prefetch_result.valid())
        prefetch_result.wait();

    auto self = weak_from_this();
    
    prefetch_result = std::async(std::launch::async, [stripes_to_check, self]() mutable
    {
        try
        {
            if (auto s = self.lock())
                s->checkStripeFiltered(stripes_to_check);
        }
        catch (Exception & e)
        {
...
bool NativeORCBlockInputFormat::prepareStripeReader()
{
    ...
    if (prewhere_info)
    {
        ...
        {
            // LOG_DEBUG(log, "prewhere_include_indices_for_stripe is empty, and the size of constant_columns_desc is {}.", constant_columns_desc->size());
    ...
    {
        // LOG_DEBUG(log, "include_indices_for_stripe is empty, and the size of constant_columns_desc is {}.", constant_columns_desc->size());
...
void NativeORCBlockInputFormat::addPrewhere(NamesAndTypesList prewhere_columns_, PrewhereInfoPtr prewhere_info_)
{
    if (!prewhere_info_)
        return;

    prewhere_columns = prewhere_columns_;
    prewhere_info = prewhere_info_->clone();
...
size_t NativeORCBlockInputFormat::executePrewhere(ColumnsWithTypeAndName & pre_res_columns, ColumnPtr & filter_holder, const ColumnUInt8 * & filter, bool nextStripe)
{
    ...
    auto expr_dag = std::make_shared<ExpressionActions>(prewhere_info->prewhere_actions.clone());
    expr_dag->execute(block);
...
size_t NativeORCBlockInputFormat::executeFilter(ColumnsWithTypeAndName & pre_res_columns, ColumnsWithTypeAndName & res_columns, size_t num_rows, const ColumnUInt8 * filter)
{
    if (!filter)
        return num_rows;
    ...
    if (prewhere_info->need_filter || bytes_in_filter < 0.6 * num_rows)
    {
        ...
        auto do_filter = [&](ColumnsWithTypeAndName & input_columns)
        {
...
Chunk NativeORCBlockInputFormat::read()
{
    ...
    if (prewhere_info)
    {
        ...
        if (executePrewhere(pre_res_columns, filter_holder, filter) == 0)
        {
    ...
    num_rows = executeFilter(pre_res_columns, res_columns, num_rows, filter);
...
void NativeORCBlockInputFormat::resetParser()
{
    ...
    next_prewhere_stripe_reader.reset(nullptr);
