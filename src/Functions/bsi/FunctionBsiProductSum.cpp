...
private:
        ...
        ColumnUInt64::Container & result_data)
    {
        ...
        for (size_t row = 0; row < array_offsets_i.size(); ++row)
        {
            ...
            if (not_null_tmp_rbm.roaring_bitmap_with_small_set.size() != 0)
                not_null_tmp_rbm.roaring_bitmap_with_small_set.rb_and(empty_rbm.roaring_bitmap_with_small_set);
            not_null_tmp_rbm.roaring_bitmap_with_small_set.merge(getBitmapDataRefOfBSI(array_data_i, current_i_pos).roaring_bitmap_with_small_set);
            not_null_tmp_rbm.roaring_bitmap_with_small_set.rb_and(getBitmapDataRefOfBSI(array_data_j, current_j_pos).roaring_bitmap_with_small_set);
            ...
            if (not_null_tmp_rbm.roaring_bitmap_with_small_set.size() > 0)
            {
                for (current_i_pos = row_start_i + 1; current_i_pos < i_end; ++current_i_pos)
                {
                    ...
                    {
                        /// If bitmap is not empty, clear it.
                        if (tmp_rbm.roaring_bitmap_with_small_set.size() != 0)
                            tmp_rbm.roaring_bitmap_with_small_set.rb_and(empty_rbm.roaring_bitmap_with_small_set);
                        tmp_rbm.roaring_bitmap_with_small_set.merge(getBitmapDataRefOfBSI(array_data_i, current_i_pos).roaring_bitmap_with_small_set);
                        tmp_rbm.roaring_bitmap_with_small_set.rb_and(getBitmapDataRefOfBSI(array_data_j, current_j_pos).roaring_bitmap_with_small_set);
                        ...
                        size_t sum_k_l = tmp_rbm.roaring_bitmap_with_small_set.size() * (1ULL << (k + l));
                        total_sum += sum_k_l;
                    }
                }
            }
...
REGISTER_FUNCTION(BsiProductSum)
{
