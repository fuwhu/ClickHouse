    ...
    ColumnPtr executeImpl(const ColumnsWithTypeAndName & arguments, const DataTypePtr & /*result_type*/, size_t /*input_rows_count*/) const override
    {
        ...
        {   
            ...
            for (int i = slice_number - 1; i >= 0; --i)
            {
                ...
                size_t count = bitmap_tmp.rbs.rb_or_cardinality(bitmap_res.rbs);
            ...
            size_t count = bitmap_res.rbs.rb_or_cardinality(bitmap_existence.rbs);
            if (count > data) 
            {
                AggregateFunctionGroupBitmapData<UInt64> bitmap_tmp;
                bitmap_existence.rbs.rb_limit(0, data - bitmap_res.rbs.size(), bitmap_tmp.rbs);
                bitmap_res.rbs.rb_or(bitmap_tmp.rbs);
            }
            else 
            ...
            PaddedPODArray<UInt64> id_arr;
            bitmap_res.rbs.rb_to_array(id_arr);
            Array field;
            getArrayField(id_arr, field, slice_number, col_nested, row_start);
            col_arr_res->assumeMutable()->insert(field);
            ++row;
...
private:
    void getArrayField(PaddedPODArray<UInt64> & id_arr, Array & field, size_t slice_number, const ColumnAggregateFunction * col_nested, size_t row_start) const
    {
        for (size_t i = 0; i < id_arr.size(); ++i)
        {
            UInt64 metrics = 0;
            for (int j = slice_number - 1; j >= 0; --j)
            {
                AggregateFunctionGroupBitmapData<UInt64> & bitmap_slice = *reinterpret_cast<AggregateFunctionGroupBitmapData<UInt64> *>(col_nested->getData()[row_start + j]);
                if (bitmap_slice.rbs.rb_contains(id_arr[i]))
                {
                    metrics += std::pow(2, j);
                }
            }
