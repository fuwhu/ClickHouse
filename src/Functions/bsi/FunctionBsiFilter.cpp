...
public:
        ...
        const ColumnAggregateFunction & rbm_col)
    {
        ...
        AggregateFunctionPtr bitmap_function = AggregateFunctionFactory::instance().get(
            AggregateFunctionGroupBitmapData<UInt64>::name(), NullsAction::EMPTY, bitmap_argument_types, params_row, properties);
    ...
    ColumnPtr executeImpl(const ColumnsWithTypeAndName & arguments, const DataTypePtr & result_type, size_t input_rows_count) const override
    {
        ...
        if (num_args != 2)
            throw Exception(
                ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Function {} requires 2 arguments: bsi, filter_bitmap.", getName());
