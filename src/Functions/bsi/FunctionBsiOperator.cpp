...
public:
    ...
    DataTypePtr getReturnTypeImpl(const DataTypes & arguments) const override
    {
        ...
        AggregateFunctionPtr bitmap_func = AggregateFunctionFactory::instance().get(
            AggregateFunctionGroupBitmapData<UInt64>::name(), NullsAction::EMPTY, argument_types, params_row, properties);
...
private:
    ColumnPtr executeBsiData(const ColumnsWithTypeAndName & arguments, size_t /*input_row_count*/) const
    {
        ...
        if (is_column_const)
            container
                = &typeid_cast<const ColumnUInt64 *>(typeid_cast<const ColumnConst *>(column_ptr)->getDataColumnPtr().get())->getData();
        else
        ...
        AggregateFunctionPtr bitmap_function = AggregateFunctionFactory::instance().get(
            AggregateFunctionGroupBitmapData<UInt64>::name(), NullsAction::EMPTY, argument_types, parameters, properties);
        ...
        for (const auto * end = offset_data + offsets_size; offset_data < end; ++offset_data)
        {
            ...
            AggregateFunctionGroupBitmapData<UInt64> & bitmap_res
                = *reinterpret_cast<AggregateFunctionGroupBitmapData<UInt64> *>(col_res->getData()[col_res->size() - 1]);
...
struct BsiGtImpl
{
    ...
    static void apply(
        UInt64 value,
        AggregateFunctionGroupBitmapData<UInt64> & bitmap_greater,
        AggregateFunctionGroupBitmapData<UInt64> & bitmap_existence,
        size_t slice_number,
        ...
        size_t & pos)
    {
        ...
        for (int i = static_cast<int>(slice_number) - 1; i >= 0; --i)
        {
            ...
            else
            {
                ...
                bitmap_greater.roaring_bitmap_with_small_set.rb_or(bitmap_tmp.roaring_bitmap_with_small_set);
...
struct BsiGeImpl
{
        ...
        size_t & pos)
    {
        ...
        bitmap_greater.roaring_bitmap_with_small_set.rb_or(bitmap_existence.roaring_bitmap_with_small_set);
    }
};
...
struct BsiLtImpl
{
    ...
    static void apply(
        UInt64 value,
        AggregateFunctionGroupBitmapData<UInt64> & bitmap_less,
        AggregateFunctionGroupBitmapData<UInt64> & bitmap_existence,
        size_t slice_number,
        const ColumnAggregateFunction & col_nested,
        size_t & pos)
    {
        ...
        {
            pos += slice_number;
            bitmap_less.roaring_bitmap_with_small_set.rb_or(bitmap_existence.roaring_bitmap_with_small_set);
            return;
        }
        ...
        for (int i = static_cast<int>(slice_number) - 1; i >= 0; --i)
        {
            ...
            if (exist)
            {
                ...
                bitmap_tmp.roaring_bitmap_with_small_set.rb_andnot(bitmap_data.roaring_bitmap_with_small_set);
                bitmap_less.roaring_bitmap_with_small_set.rb_or(bitmap_tmp.roaring_bitmap_with_small_set);
...
template class FunctionBsiOperator<BsiGtImpl>;
template class FunctionBsiOperator<BsiLtImpl>;
template class FunctionBsiOperator<BsiGeImpl>;
template class FunctionBsiOperator<BsiLeImpl>;
...
public:
    ...
    DataTypePtr getReturnTypeImpl(const DataTypes & arguments) const override
    {
        ...
        {
            throw Exception(
                ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Third argument column should be UInt, but it has type {}", arguments[2]->getName());
        }
...
private:
    static ColumnPtr executeBsiData(const ColumnsWithTypeAndName & arguments, size_t /*input_row_count*/)
    {
        ...
        if (is_column_const[0])
            container0
                = &typeid_cast<const ColumnUInt64 *>(typeid_cast<const ColumnConst *>(column_ptr[0])->getDataColumnPtr().get())->getData();
        else
            container0 = &typeid_cast<const ColumnUInt64 *>(column_ptr[0])->getData();

        if (is_column_const[1])
            container1
                = &typeid_cast<const ColumnUInt64 *>(typeid_cast<const ColumnConst *>(column_ptr[1])->getDataColumnPtr().get())->getData();
        else
        ...
        for (const auto * end = offset_data + offsets_size; offset_data < end; ++offset_data)
        {
            ...
            bitmap_existence_for_less.roaring_bitmap_with_small_set.rb_or(bitmap_existence.roaring_bitmap_with_small_set);
            ...
            if (data2 >> slice_number == 0)
            {
                ...
                for (int i = static_cast<int>(slice_number) - 1; i >= 0; --i)
                {
                    ...
                    {
                        AggregateFunctionGroupBitmapData<UInt64> bitmap_tmp;
                        bitmap_tmp.roaring_bitmap_with_small_set.rb_or(bitmap_existence_for_less.roaring_bitmap_with_small_set);

                        bitmap_tmp.roaring_bitmap_with_small_set.rb_andnot(bitmap_data.roaring_bitmap_with_small_set);
                        bitmap_res_for_less.roaring_bitmap_with_small_set.rb_or(bitmap_tmp.roaring_bitmap_with_small_set);
                        bitmap_existence_for_less.roaring_bitmap_with_small_set.rb_and(bitmap_data.roaring_bitmap_with_small_set);
                    }
                    else
                    {
                        bitmap_existence_for_less.roaring_bitmap_with_small_set.rb_andnot(bitmap_data.roaring_bitmap_with_small_set);
                    }
                }
                bitmap_res_for_less.roaring_bitmap_with_small_set.rb_or(bitmap_existence_for_less.roaring_bitmap_with_small_set);
                bitmap_res.roaring_bitmap_with_small_set.rb_and(bitmap_res_for_less.roaring_bitmap_with_small_set);
            }
