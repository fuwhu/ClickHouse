        ...
        ColumnUInt64::Container & result_data)
    {
        ...
        for (size_t row = 0; row < array_offsets.size(); ++row)
        {
            ...
            for (size_t i = row_start + 1; i < offset_end; ++i)
            {
                ...
                if (rbm_data.roaring_bitmap_with_small_set.size() == 0)
                    continue;

                size_t sum_k_l = rbm_data.roaring_bitmap_with_small_set.size() * (1ULL << (2 * k));
                total_sum += sum_k_l;
            }
            ...
            for (size_t i = row_start + 1; i < offset_end; ++i)
            {
                for (size_t j = i + 1; j < offset_end; ++j)
                {
                    ...
                    tmp_rbm.roaring_bitmap_with_small_set.merge(getBitmapDataRefOfBSI(array_data, i).roaring_bitmap_with_small_set);
                    tmp_rbm.roaring_bitmap_with_small_set.rb_and(getBitmapDataRefOfBSI(array_data, j).roaring_bitmap_with_small_set);
                    ...
                    size_t sum_k_l = tmp_rbm.roaring_bitmap_with_small_set.size() * (1ULL << (k + l + 1));
                    total_sum += sum_k_l;
                }
            }
...
REGISTER_FUNCTION(BsiSquareSum)
{
