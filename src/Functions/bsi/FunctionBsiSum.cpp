...
public:
            ...
            for (; pos < offset_data; ++pos)
            {
                ...
                if (row_offset == 0)
                {
                    ...
                    if (rbm_col)
                    {
                        ...
                        count_value = bitmap_data.roaring_bitmap_with_small_set.rb_and_cardinality(rbm_data.roaring_bitmap_with_small_set);
                    }
                    else
                        count_value = bitmap_data.roaring_bitmap_with_small_set.size();
                }
                else if (row_offset > 0)
                {
                    ...
                    if (rbm_col)
                    {
                        ...
                        UInt64 rb_and_cardinality
                            = bitmap_data.roaring_bitmap_with_small_set.rb_and_cardinality(rbm_data.roaring_bitmap_with_small_set);
                        sum_value += rb_and_cardinality * static_cast<UInt64>(std::pow(2, row_offset - 1));
                    }
                    else
                    {
                        UInt64 rb_cardinality = bitmap_data.roaring_bitmap_with_small_set.size();
                        sum_value += rb_cardinality * static_cast<UInt64>(std::pow(2, row_offset - 1));
                    }
                }
    ...
    ColumnPtr executeImpl(const ColumnsWithTypeAndName & arguments, const DataTypePtr & result_type, size_t input_rows_count) const override
    {
        ...
        if (num_args < 1)
            throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Function {} requires at least one arguments", getName());

        if (num_args > 2)
            throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Function {} requires at most two parameters", getName());
