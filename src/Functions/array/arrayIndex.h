...
public:
    ...
    DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName & arguments) const override
    {
        ...
        if constexpr (std::is_same_v<ConcreteAction, HasAction>)
        {
            ...
            inner_type = map_v2_type ? map_v2_type->getKeyType() : (map_type ? map_type->getKeyType() : array_type->getNestedType());
        }
        else
        {
            if (!array_type)
        ...
        {
            const char * first_argument_type_name = map_v2_type ? "mapv2" : (map_type ? "map" : "array");
            throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
    ...
    {
        if (auto res = executeMapV2(arguments, result_type))
            return res;
...
#undef INTEGRAL_PACK
    ...
    ColumnPtr executeMapV2(const ColumnsWithTypeAndName & arguments, const DataTypePtr & result_type) const
    {
        if constexpr (!std::is_same_v<ConcreteAction, HasAction>)
            return nullptr;
