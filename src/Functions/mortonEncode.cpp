...
\
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal number of UInt arguments of function {}, max: 8", getName());
...
DECLARE_DEFAULT_CODE(
    ...
    public:
        ...
        DataTypePtr getReturnTypeImpl(const DB::DataTypes & arguments) const override
        {
            ...
            if (arguments.empty())
                throw Exception(
                    ErrorCodes::TOO_FEW_ARGUMENTS_FOR_FUNCTION, "At least one UInt argument is required for function {}", getName());
                ...
                for (size_t i = 0; i < tuple_size; i++)
                {
                    if (!WhichDataType(type_tuple->getElement(i)).isNativeUInt())
                        throw Exception(
                            ...
                            type_tuple->getElement(i)->getName(),
                            getName());
                }
            }
...
public:
    explicit FunctionMortonEncode(ContextPtr context) : selector(context)
    {
        ...
        selector.registerImplementation<TargetArch::AVX2, TargetSpecific::AVX2::FunctionMortonEncode>();
#endif
    }
...
void registerFunctionMortonEncode(FunctionFactory & factory)
{
    ...
    factory.registerAlias("zCurveEncode", FunctionMortonEncode::name);
    factory.registerAlias("zCurve", FunctionMortonEncode::name);
}
}
