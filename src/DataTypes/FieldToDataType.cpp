...
DataTypePtr FieldToDataType::operator() (const MapV2 & map) const
{
    DataTypes key_types;
    DataTypes value_types;
    key_types.reserve(map.size());
    ...
    return std::make_shared<DataTypeMapV2>(getLeastSupertype(key_types), getLeastSupertype(value_types));
}
