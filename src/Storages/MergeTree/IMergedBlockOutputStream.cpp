    ...
    MergeTreeData::DataPart::Checksums & checksums)
{
    ...
    for (const auto & column : columns)
    {
        data_part->getSerialization(column.name)->enumerateStreams(
            ...
            {
                auto stream_name = IMergeTreeDataPart::getStreamNameForColumn(column, substream_path, checksums, data_part->getHashCollisionMap());
                if (stream_name)
    ...
    for (const auto & column_name : empty_columns)
    {
        ...
        {
            auto stream_name = IMergeTreeDataPart::getStreamNameForColumn(column_name, substream_path, checksums, data_part->getHashCollisionMap());
