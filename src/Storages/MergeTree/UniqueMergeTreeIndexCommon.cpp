...
namespace DB
{
void DeletedKeys::serializeBinary(WriteBuffer & ostr, bool is_write_binary) const
{
...
void DeletedKeys::deserializeBinary(ReadBuffer & istr, bool is_read_binary)
{
    size_t size;
...
void UniqueKeyBucketIndex::init(size_t bucket_size_, size_t rows_count_)
{
    bucket_size = bucket_size_;
    bucket_num = static_cast<size_t>(std::ceil(rows_count_ * 1.0 / bucket_size));

    LOG_DEBUG(getLogger("UniqueMergeTreeIndex"), "bucket bucket_size {} rows_count {} bucket_num {}", bucket_size, rows_count_, bucket_num);
}
...
void StringHashMapUniqueKeyIndex::initBucket(size_t bucket_num_)
{
...
std::vector<size_t> StringHashMapUniqueKeyIndex::calculateTargetBuckets(size_t mod_bucket_num) const
{
...
{
    auto file_size = data_part_storage.getFileSize(file_name);
    auto read_settings = ReadSettings().adjustBufferSize(file_size);

    if (!bucketing_enabled)
    {
        auto istr = data_part_storage.readFile(file_name, read_settings, file_size, {});
        size_t size;
    ...
    else
    {
        ...
        if (bucket_range)
            LOG_DEBUG(getLogger("UniqueMergeTreeBucketIndex"), "will load {}/{} bucket index", bucket_range->size(), bucket_num);
        ...
        for (size_t i = 0; i < bucket_num; i++)
        {
                ...
                else
                {
                    loading_bucket_pool->scheduleOrThrowOnError(
                        [&, this, i, offset_in_file, rows, thread_group = CurrentThread::getGroup()]
                        {
                            ...
                            try
                            {
                                auto index_istr = data_part_storage.readFile(file_name, read_settings, file_size, {});
            ...
            if (!schedule_flag)
                throw Exception(
                    ...
                    "Timeout while scheduling loading buckets of unique key index, timeout is {}, and BackgroundUniqueEngineLoadTask is {}",
                    timeout_in_sec,
                    busy_threads_in_pool);
        }
...
void StandardMapUniqueKeyIndex::initBucket(size_t bucket_num_)
{
    bucketing_enabled = true;
    bucket_num = bucket_num_;
    unique_key_index_bucket.resize(bucket_num_);
...
void StandardMapUniqueKeyIndex::add(const String & key, const VersionAndRow & value)
{
    ...
    else
    {
        int bucket_idx = 0;
        if (bucket_num > 1)
            bucket_idx = static_cast<int>(CityHash_v1_0_2::CityHash64(key.data(), key.size()) % bucket_num);
        unique_key_index_bucket[bucket_idx][key] = value;
    }
}
...
std::vector<size_t> StandardMapUniqueKeyIndex::calculateTargetBuckets(size_t mod_bucket_num) const
{
    std::vector<size_t> bucket_index_range;
...
void StandardUnOrderedMapUniqueKeyIndex::initBucket(size_t bucket_num_)
{
    bucketing_enabled = true;
    bucket_num = bucket_num_;
    unique_key_index_bucket.resize(bucket_num_);
...
std::vector<size_t> StandardUnOrderedMapUniqueKeyIndex::calculateTargetBuckets(size_t mod_bucket_num) const
{
    std::vector<size_t> bucket_index_range;
...
void StandardUnOrderedMapUniqueKeyIndex::deserializeBinary(
    ...
    size_t timeout_in_sec)
{
    auto file_size = data_part_storage.getFileSize(index_path);
    auto read_settings = ReadSettings().adjustBufferSize(file_size);

    if (!bucketing_enabled)
    {
        auto istr = data_part_storage.readFile(index_path, read_settings, file_size, {});
        size_t size;
        DB::readBinary(size, *istr);
                ...
                else
                {
                    loading_bucket_pool->scheduleOrThrowOnError(
                        [&, this, i, offset_in_file, rows, thread_group = CurrentThread::getGroup()]
                        {
                            ...
                            try
                            {
                                auto index_istr = data_part_storage.readFile(index_path, read_settings, file_size, {});
                                index_istr->seek(offset_in_file, SEEK_SET);
                                UniqueKeyIndexMapType unique_key_version_tmp;
                                for (size_t rows_idx = 0; rows_idx < rows; rows_idx++)
                                {
...
std::optional<VersionAndRow> LevelDBUniqueKeyIndex::get(const String & key, bool rowid_is_uinit32) const
{
    String value;
    ...
    if (status.ok())
    {
        ...
        throw Exception(ErrorCodes::UNKNOWN_EXCEPTION, "Failed to lookup key: {}", status.ToString());
}
    ...
    bool is_same_key) const
{
    ...
    if (rows == 0)
    {
        LOG_DEBUG(getLogger("UniqueMergeTreeIndex"), "index type LevelDBUniqueKeyIndex block rows is zero.");
        return;
    }
    ...
    if (!is_same_key)
    {
        ...
        unique_sort_description.emplace_back(SortColumnDescription{UNIQUE_VIRTUAL_KEY_COLUMN_NAME, 1, 1});
        ...
        if (!isAlreadySorted(block, unique_sort_description))
        {
            ...
            LOG_DEBUG(getLogger("UniqueMergeTreeIndex"), "unique key is already sorted.");
    }
    else
        LOG_DEBUG(getLogger("UniqueMergeTreeIndex"), "unique key is the same as order key and doesn't need to be sorted.");
    ...
    std::unique_ptr<rocksdb::DB> & tmp_rocksdb_index_writer) const
{
    ...
    if (!iter->status().ok())
        throw Exception(
            ErrorCodes::LOGICAL_ERROR, "Error while scanning temp key index file {}: {}", tmp_rocksdb_index_dir, iter->status().ToString());
    iter.reset();
...
void LevelDBUniqueKeyIndex::deserializeBinary(const String & index_path, UniqueKeyIndexBlockCachePtr block_cache)
{
    IndexFile::Options options;
    options.block_cache = std::move(block_cache);
    auto local_reader = std::make_unique<IndexFile::IndexFileReader>(options);
    auto status = local_reader->Open(index_path);
    if (!status.ok())
        throw Exception(ErrorCodes::UNKNOWN_EXCEPTION, "Failed to open index file {}: {}", index_path, status.ToString());
...
UniqueKeyIterator LevelDBUniqueKeyIndex::newIterator(const IndexFile::ReadOptions & options) const
{
    ...
    if (!st.ok())
        throw Exception(ErrorCodes::UNKNOWN_EXCEPTION, "Failed to get iterator: {}", st.ToString());
    return res;
}
