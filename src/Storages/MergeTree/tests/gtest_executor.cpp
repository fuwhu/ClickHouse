...
public:
    explicit LambdaExecutableTask(const String & name_, size_t step_count_, StepFunc step_func_ = {}, UInt64 priority_ = 0)
        : name(name_), step_count(step_count_), step_func(step_func_), priority(priority_)
    {
    }
    ...
    StorageID getStorageID() override { return {"test", name}; }
...
TEST(Executor, RemoveTasks)
{
    ...
    auto executor = DB::MergeTreeBackgroundExecutor<RoundRobinRuntimeQueue>::create
    (
        "GTest",
        tasks_kinds,
        tasks_kinds * batch,
