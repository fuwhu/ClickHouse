...
public:
    ...
    void updatePolicy(std::string_view) { }
...
public:
    TaskRuntimeDataPtr pop()
    {
        return std::visit([&](auto && queue) { return queue.pop(); }, impl);
    }
    ...
    bool empty()
    {
        return std::visit([&](auto && queue) { return queue.empty(); }, impl);
    }
    ...
    void updatePolicy(std::string_view name)
    {
        ...
        resolve<Policies...>(
            name,
            [&]<class NewQueue>(std::in_place_type_t<NewQueue>)
            {
                std::visit(
                    [&](auto && queue)
                    {
...
public:
    ...
    MergeTreeBackgroundExecutor(
        String name_, size_t threads_count_, size_t max_tasks_count_, CurrentMetrics::Metric metric_, std::string_view policy) requires
        requires(Queue queue)
    {
        queue.updatePolicy(policy);
    ...
    void updateSchedulingPolicy(std::string_view new_policy) requires requires(Queue queue)
    {
        queue.updatePolicy(new_policy);
    } // Because we use explicit template instantiation
    {
