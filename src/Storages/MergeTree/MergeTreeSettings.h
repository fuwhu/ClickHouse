...
#define LIST_OF_MERGE_TREE_SETTINGS(M) \
    ...
    M(UInt64, unique_delete_bitmap_type, 64, "The type of delete bitmap used by unique engine. 64 - Roaring64Bitmap; 32 - Roaring32Bitmap.", 0) \
    M(UInt64, unique_key_index_type, 3, "The type of index used by unique engine for unique key. 0 - StandardMapUniqueKeyIndex; 1 - StandardUnOrderedMapUniqueKeyIndex; 2 - StringHashMapUniqueKeyIndex; 3 - LevelDB.", 0) \
    ...
    M(UInt64, unique_key_deduplicate_level, 0, "The level of deduplicate rows for unique engine. 0 - table level; 1 - partition level", 0) \
    ...
    M(Bool, enable_unique_key_partition_lock, 1, "Enable the unique key partition lock, if this value is set to 0, even if the table is deduplicated at the partition level, the lock granularity will be at the table level.", 0) \
    M(UInt64, unique_key_partition_lock_limit, 10000, "Maximum number of partition-level locks maintained for unique engine tables. When the limit is reached and a new partition is written, an exception is thrown. Existing partitions are not affected. This setting is meaningful only when enable_unique_key_partition_lock is set to 1.", 0) \
    \
    ...
    M(Bool, implicit_map_duplication, false, "Whether to store redundancy map data for selecting whole MapV2 column.", 0) \
    ...
    M(UInt64, max_implicit_columns, 300, "If more than this number implicit columns in MapV2, throw 'Too many implicit columns ...' exception.", 0) \
