...
{
    void serializeBinary(WriteBuffer & ostr, bool is_write_binary) const;

    void deserializeBinary(ReadBuffer & istr, bool is_read_binary);
};
...
struct UniqueKeyBucketIndex : IUniqueKeyBucketIndex
{
    ...
    void init(size_t bucket_size_, size_t rows_count_) override;
...
struct StringHashMapUniqueKeyIndex : public IUniqueKeyIndex
{
    ...
    void initBucket(size_t bucket_num_) override;
    ...
    std::vector<size_t> calculateTargetBuckets(size_t mod_bucket_num) const override;
...
struct LevelDBUniqueKeyIndex : public IUniqueKeyIndex
{
    ...
    std::optional<VersionAndRow> get(const String & key, bool rowid_is_uinit32) const override;
    ...
    void serializeBinary(
        ...
        bool is_same_key) const override;

    void serializeBinary(
        const String & index_path,
        IndexFile::IndexFileInfo & file_info,
        const String & tmp_rocksdb_index_dir,
        std::unique_ptr<rocksdb::DB> & tmp_rocksdb_index_writer) const override;

    void deserializeBinary(const String & index_path, UniqueKeyIndexBlockCachePtr block_cache) override;
