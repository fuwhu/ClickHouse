...
void IcebergORCFile::prepare(const ContextPtr & context, const ReadType & read_type)
{
    ...
    auto * orc_input_format = getORCInputFormat();
    if (!orc_input_format->getSharedBuffer())
...
{
    if (!input_format)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "orc input format is null while getting orc input format.");
    auto * orc_input_format = dynamic_cast<NativeORCBlockInputFormat *>(input_format.get());
    if (!orc_input_format)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "orc input format is null after dynamic casting while orc_input_format is ORC.");
