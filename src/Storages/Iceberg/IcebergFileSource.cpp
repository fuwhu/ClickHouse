...
namespace CurrentMetrics
{
    ...
    extern const Metric ORCPreInitThreads;
    extern const Metric ORCPreInitThreadsActive;
    extern const Metric ORCPreInitThreadsScheduled;
}
...
namespace Setting
{
    extern const SettingsBool iceberg_data_cache_enabled;
    extern const SettingsMaxThreads iceberg_file_input_format_pre_initialization_thread_pool_size;
    extern const SettingsFloat iceberg_file_pre_initialization_batch_size_pool_size_ratio;
    extern const SettingsUInt64 iceberg_init_pool_max_size;
    extern const SettingsBool iceberg_file_input_format_pre_initialization;
}
...
static void checkKerberosAuth(const Poco::Util::AbstractConfiguration & config)
{
    ...
    if (hadoop_kerberos_keytab.empty() || hadoop_kerberos_principal.empty() || hadoop_security_kerberos_ticket_cache_path.empty())
        throw Exception(ErrorCodes::UNKNOWN_ELEMENT_IN_CONFIG, "Not enough parameters to check kerberos authentication");
    ...
    auto run_klist = [&]() -> bool
    {
        ...
        if (status)
            throw Exception(ErrorCodes::BAD_ARGUMENTS, "klist failure: {}", klist_cmd);
    ...
    auto run_kinit = [&]() -> void
    {
        ...
        auto status = command->tryWait();
        if (status)
            throw Exception(ErrorCodes::BAD_ARGUMENTS, "kinit failure: {}", kinit_cmd);
    };
...
namespace
{
    ...
    std::shared_ptr<ReadBufferFromFileBase> getReadBufferForFile(const IcebergDataFile & data_file, [[maybe_unused]] const ContextPtr & context, FileCachePtr cache, size_t size)
    {
        ...
        if (kind == FileSystemKind::HDFS)
        {
            ...
            bool data_cache_enabled = context->getSettingsRef()[Setting::iceberg_data_cache_enabled].value;
            ...
            if (data_cache_enabled && cache != nullptr)
            {
                ...
                //     }
                //     return read_buffer_hdfs;
                //     // return std::shared_ptr<ReadBufferFromHDFS>(read_buffer_hdfs.release(), BorrowedUniquePtrDeleteFun(pool));
                // };
                return std::make_shared<CachedOnDiskReadBufferFromFile>(data_file.path, FileCacheKey::fromPath(data_file.path), cache, FileCache::getCommonUser(), remote_file_reader_creator, setting, std::string(CurrentThread::getQueryId()), size, true, false, size, Context::getGlobalContextInstance()->getFilesystemCacheLog());
...
IcebergFileSource::IcebergFileSource(
    ...
    , log(getLogger("IcebergFileSource"))
{
    initialize();
}

std::unique_ptr<RecycleThreadPoolThread<ThreadFromGlobalPoolNoTracingContextPropagation>> DB::IcebergFileSource::static_recycle_thread;
...
void IcebergFileSource::initialize()
{
    ...
    LOG_DEBUG(log, "In current stream, totally assigned {} files with {} splits and {} bytes to read.", total_assigned_file_count, total_assigned_split_count, total_assigned_file_size);

    if (read_type == ReadType::InReverseOrder)
        std::reverse(iceberg_files->begin(), iceberg_files->end());

    const auto & settings = getContext()->getSettingsRef();
    UInt64 input_format_pre_init_pool_size = settings[Setting::iceberg_file_input_format_pre_initialization_thread_pool_size];
    if (settings[Setting::iceberg_file_input_format_pre_initialization])
    {
        input_format_preinit_pool = std::make_unique<ThreadPool>(CurrentMetrics::ORCPreInitThreads, CurrentMetrics::ORCPreInitThreadsActive, CurrentMetrics::ORCPreInitThreadsScheduled, std::min(static_cast<size_t>(input_format_pre_init_pool_size), iceberg_files->size()));
        max_pre_init_batch_size = static_cast<size_t>(input_format_preinit_pool->getMaxThreads() * settings[Setting::iceberg_file_pre_initialization_batch_size_pool_size_ratio]);
        if (!static_recycle_thread)
        {
            std::lock_guard lock(static_thread_mutex);
            if (!static_recycle_thread)
                static_recycle_thread = std::make_unique<RecycleThreadPoolThread<ThreadFromGlobalPoolNoTracingContextPropagation>>(static_cast<size_t>(settings[Setting::iceberg_init_pool_max_size]), [](size_t size) { CurrentMetrics::set(CurrentMetrics::IcebergFileSourceRecyclePoolSize, size); });
        }
    }
...
void IcebergFileSource::preInitFile(const ContextPtr & local_context)
{
    ...
    for (UInt64 i = 0; i < pre_init_batch_size; ++i)
    {
        ...
        auto init_fun = [read_buf = file->read_buf, thread_group = CurrentThread::getGroup()]
        {
            ...
            if (ReadBufferFromHDFS * hdfs_buf = dynamic_cast<ReadBufferFromHDFS *>(read_buf.get()))
                hdfs_buf->initRemoteOnlyOnce();
            else
            {
                CachedOnDiskReadBufferFromFile * remote_buf = dynamic_cast<CachedOnDiskReadBufferFromFile *>(read_buf.get());
                if (!remote_buf)
                    throw Exception(ErrorCodes::LOGICAL_ERROR, "cannot cast to CachedOnDiskReadBufferFromFile");
                
                auto creator = remote_buf->getCreator();
                if (creator)
                    creator();
            }
        };

        if (ReadBufferFromHDFS * buf = dynamic_cast<ReadBufferFromHDFS *>(file->read_buf.get()))
        {
            can_lazy_init &= buf->canLazyInit();
...
void IcebergFileSource::onCancel() noexcept
{
...
Chunk IcebergFileSource::generate()
{
    ...
    while (true)
    {
        ...
        if (chunk_generated)
        {
            ...
            ReadBufferFromHDFS * read_buffer = dynamic_cast<ReadBufferFromHDFS *>(current_file->read_buf.get());
            CachedOnDiskReadBufferFromFile * cached_read_buffer = dynamic_cast<CachedOnDiskReadBufferFromFile *>(current_file->read_buf.get());

            if (read_buffer || cached_read_buffer)
            {
                getContext()->updateIcebergFileMetrics(
                    metrics_index_in_context,
                    file_iceberg_file_source_read_time_cost_us,
                    read_buffer ? read_buffer->getRemoteReadBytes() : cached_read_buffer->getRemoteReadBytes(),
                    read_buffer ? read_buffer->getRemoteReadTimeCostMicrosecond() : cached_read_buffer->getRemoteReadTimeCostMicrosecond(),
                    read_buffer ? read_buffer->getRemoteSeekCount() : cached_read_buffer->getRemoteSeekCount(),
                    read_buffer ? read_buffer->getRemoteReadCount() : cached_read_buffer->getRemoteReadCount(),
                    read_buffer ? read_buffer->getRemoteSeekTimeCostMicrosecond() : cached_read_buffer->getRemoteSeekTimeCostMicrosecond(),
                    read_buffer ? read_buffer->getRemoteReadInitWaitCostMicrosecond() : cached_read_buffer->getRemoteReadInitWaitCostMicrosecond(),
                    read_buffer ? read_buffer->getLocalReadBytes() : cached_read_buffer->getLocalReadBytes(),
                    read_buffer ? read_buffer->getLocalReadCount() : cached_read_buffer->getLocalReadCount(),
                    read_buffer ? read_buffer->getLocalReadTimeCostMicrosecond() : cached_read_buffer->getLocalReadTimeCostMicrosecond(),
                    read_buffer ? read_buffer->getLocalWriteBytes() : cached_read_buffer->getLocalWriteBytes(),
                    read_buffer ? read_buffer->getLocalWriteCount() : cached_read_buffer->getLocalWriteCount(),
                    read_buffer ? read_buffer->getLocalWriteTimeCostMicrosecond() : cached_read_buffer->getLocalWriteTimeCostMicrosecond(),
                    read_buffer ? read_buffer->getTotalReadTimeCostMicrosecond() : cached_read_buffer->getTotalReadTimeCostMicrosecond(),
    ...
    FileCachePtr cache)
{
    ...
    if (query_info.prewhere_info)
    {
        ...
        if (!pre_column_names.empty())
        {
            ...
            iceberg_file->prewhere_info = prewhere_info;
        }
    }  
...
IcebergFileSource::splitIcebergFile(IcebergFilePtr iceberg_file, size_t request_splits, const ContextPtr & local_context, bool is_read_in_order)
{
    ...
    if (auto * old_orc_file = dynamic_cast<IcebergORCFile *>(iceberg_file.get()))
    {
        ...
        if (request_splits > old_stripes->size())
            throw Exception(ErrorCodes::LOGICAL_ERROR, "Can not split Iceberg file because request splits size is greater than ORC stripes size");
...
{
    /// order key
    SortDescription order_key_description = input_order_info->sort_description_for_merging;
    
    if (order_key_description.size() != 1)
        throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Currently only one sorting key is supported.");
    ...
    FileComparator comparator = [&](const IcebergFilePtr & lhs, const IcebergFilePtr & rhs)
    {
        ...
        if (!orc_file_lhs) 
            throw Exception(ErrorCodes::LOGICAL_ERROR, "The file format cannot be other format except orc.");
...
void IcebergFile::initializeInputFormat(const ContextPtr & context_)
{
    if (!readBufferInitialized())
    {
        ...
        auto * hdfs_buf = dynamic_cast<ReadBufferFromHDFS *>(read_buf.get());
        if (hdfs_buf && !hdfs_buf->canLazyInit())
        {
            Stopwatch watch;
            hdfs_buf->initRemoteOnlyOnce();
            watch.stop();
        ...
        {
            auto * cached_buf = dynamic_cast<CachedOnDiskReadBufferFromFile *>(read_buf.get());
            if (cached_buf)
            {
                auto creator = cached_buf->getCreator();
                if (creator)
                {
                    Stopwatch watch;
                    creator();
...
void IcebergFile::createInputFormat(const ContextPtr & context_)
{
    ...
    if (orc_input_format)
    {
        ...
        orc_input_format->setSharedBuffer(read_buf);
    }
}
