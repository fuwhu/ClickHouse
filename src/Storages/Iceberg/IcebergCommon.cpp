...
namespace
{
    ...
    Poco::Dynamic::Var visitField(const Field & field)
    {
        ...
        switch (which)
        {
            ...
            case Field::Types::Array: {
                ...
                auto tuple = field.safeGet<Array>();
    ...
    T call(const String & endpoint, const String & path, const String & data, P processor)
    {
        ...
        ConnectionTimeouts timeouts;
        timeouts.withConnectionTimeout(Poco::Timespan(3000000));
        timeouts.withSendTimeout(Poco::Timespan(10000000));
        timeouts.withReceiveTimeout(Poco::Timespan(30000000));
        ...
        auto send_with_retry = [&]() -> std::istream &
        {
            ...
            while (true)
            {
                ...
                try
                {
                    session = makeHTTPSession(HTTPConnectionGroupType::HTTP, Poco::URI(endpoint), timeouts);
        ...
        if (response.getStatus() != Poco::Net::HTTPResponse::HTTP_OK)
        {
            ...
            throw Exception::createRuntime(ErrorCodes::ICEBERG_CATALOG_ERROR, exception_message);
        }
