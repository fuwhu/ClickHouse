...
FileCachePtr getCachePtrForDisk(
    ...
    ContextPtr context)
{
    bool data_cache_enabled = context->getSettingsRef().get(name + "_data_cache_enabled").safeGet<bool>();
    if (!data_cache_enabled)
        return nullptr;

    FileCacheSettings settings;
    settings.loadIcebergCacheFromConfig(config, config_prefix);

    // auto cache = FileCacheFactory::instance().getOrCreate(settings);
    auto cache = FileCacheFactory::instance().getOrCreate(name, settings, name);
    cache->initialize();
    return cache;
}
